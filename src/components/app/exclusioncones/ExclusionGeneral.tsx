...
const ExclusionGeneral: React.FC = () => {
    ...
    >(null);
    const [satellites, setSatellites] = useState<ExclusionCones[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [editingSatellite, setEditingSatellite] = useState<ExclusionCones | null>(null);
    ...
    useEffect(() => {
        const fetchSatellites = async () => {
            setLoading(true);
            try {
                const response = await axios.get<ExclusionCones[]>(
                    "/api/v1/satellites/"
                );
    ...
    const handleEdit = (satellite: ExclusionCones) => {
    ...
    const sortedSatellites = [...satellites].sort((a, b) => {
        ...
        const aValue = a[sortConfig.key as keyof ExclusionCones];
        const bValue = b[sortConfig.key as keyof ExclusionCones];
